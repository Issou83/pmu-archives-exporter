        ...
        if (match) {
          ...
          if (numbers.length >= 3) {
            ...
            if (validNumbers.length >= 3) {
              ...
              console.log(
                `   ✅ Rapport trouvé dans /arrivees-rapports/: ${reportFromArrival}`
              );
            }
          }
        }
            ...
            if (numbersMatch) {
              ...
              if (numbers.length >= 3) {
                ...
                if (validNumbers.length >= 3) {
                  reportFromArrival = validNumbers.join('-');
                  console.log(
                    `   ✅ Rapport trouvé dans body: ${reportFromArrival}`
                  );
                  break;
                }
              }
        ...
        if (match) {
          ...
          if (numbers.length >= 3) {
            ...
            if (validNumbers.length >= 3) {
              ...
              console.log(
                `   ✅ Rapport trouvé dans /partants-programmes/: ${reportFromOriginal}`
              );
            }
          }
        }
            ...
            if (numbersMatch) {
              ...
              if (numbers.length >= 3) {
                ...
                if (validNumbers.length >= 3) {
                  reportFromOriginal = validNumbers.join('-');
                  console.log(
                    `   ✅ Rapport trouvé dans body: ${reportFromOriginal}`
                  );
                  break;
                }
              }
    ...
    if (foundReport) {
      ...
      if (expectedReport) {
        if (foundReport === expectedReport) {
          console.log(
            `   ✅ Correspond avec le rapport scrapé: ${expectedReport}`
          );
        } else {
      ...
      if (expectedReport) {
        console.log(
          `   ⚠️  PROBLÈME: Rapport scrapé (${expectedReport}) mais pas trouvé à la source !`
        );
      } else {
        console.log(
          `   ✅ Confirmé: Pas de rapport disponible (scrapé: "Non disponible")`
        );
      }
    }
...
async function testMultipleUrls() {
  ...
  resultsWith.forEach((r, i) => {
    if (r.foundReport) {
      if (r.matches) {
        console.log(
          `   ${i + 1}. ✅ ${r.url.split('/').pop()}: ${r.foundReport} (correspond)`
        );
      } else {
        console.log(
          `   ${i + 1}. ⚠️  ${r.url.split('/').pop()}: ${r.foundReport} (différent de ${r.expectedReport})`
        );
      }
    } else {
      console.log(
        `   ${i + 1}. ❌ ${r.url.split('/').pop()}: Pas trouvé (attendu: ${r.expectedReport})`
      );
    }
  });
  ...
  resultsWithout.forEach((r, i) => {
    if (r.foundReport) {
      console.log(
        `   ${i + 1}. ⚠️  ${r.url.split('/').pop()}: ${r.foundReport} (PROBLÈME: trouvé alors qu'on pensait qu'il n'y en avait pas !)`
      );
    } else {
      console.log(
        `   ${i + 1}. ✅ ${r.url.split('/').pop()}: Confirmé - Pas de rapport`
      );
    }
  });
  ...
  const incorrectWith = resultsWith.filter(
    (r) => r.foundReport && !r.matches
  ).length;
  const missingWith = resultsWith.filter(
    (r) => !r.foundReport && r.expectedReport
  ).length;
  ...
  console.log(
    `      ⚠️  Faux négatifs: ${incorrectWithout}/${resultsWithout.length}`
  );
}
