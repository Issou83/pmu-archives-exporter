...
async function testAPIAndVerifyData() {
  ...
  try {
    const response = await fetch(
      `${API_URL}?source=turf-fr&years=2024&months=janvier`
    );
    ...
    if (!Array.isArray(result)) {
      console.log(
        `   ❌ L'API a retourné une erreur: ${JSON.stringify(result)}`
      );
      return;
    }
    ...
    if (missingFields.length > 0) {
      console.log(
        `   ⚠️  ${missingFields.length} réunions avec champs manquants`
      );
    ...
    if (invalidDates.length > 0) {
      console.log(
        `   ⚠️  ${invalidDates.length} réunions avec dates invalides ou hors janvier 2024`
      );
    ...
    console.log(
      `   Rapports d'arrivée: ${withReports.length} avec, ${withoutReports.length} sans`
    );

    if (withoutReports.length > 0) {
      console.log(
        `   ⚠️  ${withoutReports.length} réunions sans rapport d'arrivée`
      );
    ...
    if (invalidHippodromes.length > 0) {
      console.log(
        `   ⚠️  ${invalidHippodromes.length} réunions avec hippodromes invalides`
      );
      invalidHippodromes.slice(0, 5).forEach((r) => {
        console.log(
          `      - Hippodrome: "${r.hippodrome}" (R${r.reunionNumber})`
        );
      });
    } else {
    ...
    const invalidCountries = data.filter((r) => {
      return (
        !r.countryCode ||
        !['FR', 'GB', 'SWE', 'USA', 'IRE', 'GER', 'ITA'].includes(r.countryCode)
      );
    });
    ...
    data.slice(0, 5).forEach((r) => {
      console.log(
        `      - ${r.hippodrome} R${r.reunionNumber}: ${r.dateLabel} | Rapport: ${r.arrivalReport || 'N/A'}`
      );
    });
  } catch (error) {
